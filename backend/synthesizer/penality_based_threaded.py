...
class Synthesizer:
    ...
    def find_patterns_with_gpt(self):
        """Generate patterns using GPT instead of the rule-based approach."""
        # Check if GPT service is available
        if not self.gpt_service.is_available():
            print("GPT service not available, falling back to original pattern synthesis")
            return self._find_patterns_original()
            
        try:
            # Convert examples to list of strings if they're not already
            positive_examples = self.positive_examples
            negative_examples = self.negative_examples
            
            # Limit the number of examples to avoid overloading the API
            MAX_EXAMPLES = 10  # Increased from 5 to 10 as requested
            if len(positive_examples) > MAX_EXAMPLES:
                # Sample a subset of examples if we have too many
                positive_sample = random.sample(positive_examples, MAX_EXAMPLES)
            else:
                positive_sample = positive_examples
                
            # Limit negative examples too
            if negative_examples and len(negative_examples) > MAX_EXAMPLES:
                negative_sample = random.sample(negative_examples, MAX_EXAMPLES)
            else:
                negative_sample = negative_examples
            
            # Call GPT service to generate patterns
            print("Calling GPT service to generate patterns...")
            gpt_patterns = self.gpt_service.generate_patterns(positive_sample, negative_sample)
            
            # Handle empty responses
            if not gpt_patterns:
                print("No patterns returned from GPT, falling back to original pattern synthesis")
                return self._find_patterns_original()
                
            # Convert GPT patterns to the format expected by the application
            patterns = {}
            for i, pattern in enumerate(gpt_patterns):
                if isinstance(pattern, dict) and "pattern" in pattern:
                    pattern_id = f"gpt_pattern_{i}"
                    patterns[pattern_id] = {
                        "pattern": pattern["pattern"],
                        "explanation": pattern["explanation"],
                        "matched_terms": pattern.get("matched_terms", []),
                        "status": 0,  # Initial status (not selected)
                        "source": "gpt",
                        "weight": 0.0  # Initial weight
                    }
            
            # If no patterns were generated, fall back to original method
            if not patterns:
                print("No valid patterns generated by GPT, falling back to original pattern synthesis")
                return self._find_patterns_original()
                
            print(f"Successfully generated {len(patterns)} patterns using GPT")
            self.patterns_set = patterns
            return patterns
            
        except Exception as e:
            # Catch any unexpected errors and fall back to original method
            print(f"Error in GPT pattern generation: {str(e)}")
            return self._find_patterns_original()
    
    def find_patters(self, outfile=None):
        """Override the original pattern finding method to use GPT."""
        # First attempt to use original method (as fallback)
        if hasattr(self, 'use_original_method') and self.use_original_method:
            return self._find_patterns_original()
            
        # Use GPT for pattern generation
        return self.find_patterns_with_gpt()
    
    # Add the original pattern finding method for fallback
    def _find_patterns_original(self):
        """The original pattern finding method, kept for fallback."""
        # Set a clean patterns_set
        self.patterns_set = {}
